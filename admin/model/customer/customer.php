...
class ModelCustomerCustomer extends Model {
	...
	public function addHistory($customer_id, $title, $comment) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "customer_history SET customer_id = '" . (int)$customer_id . "', comment = '" . $this->db->escape(strip_tags($comment)) . "', title = '" . $this->db->escape(strip_tags($title)) . "', user_id = '" . $this->user->getId() . "', date_added = NOW()");
	}
	...
	public function getHistories($customer_id, $start = 0, $limit = 10) {
		...
		$query = $this->db->query("SELECT title, comment, date_added FROM " . DB_PREFIX . "customer_history WHERE customer_id = '" . (int)$customer_id . "' ORDER BY date_added DESC LIMIT " . (int)$start . "," . (int)$limit);
